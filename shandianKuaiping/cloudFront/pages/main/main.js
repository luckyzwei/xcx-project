...
Page({
  ...
  data: {
    onload:false,
    articleId: "54c8ecef-54ff-4d9d-9834-80b70edb73d8",
  ...
  onLoad: function(options) {
    ...
    this.setData({
      articleId: options.articleId ? options.articleId : this.data.articleId,
    })
 ...
 */
  ...
  getArticleInfo: function() {
     ...
     * 获取页面详情接口
     */
    getArticleInfo(articleParams).then(res => {
      ...
      let interactionList = res.resultContent.interactionList;
      var hasTopic = false;
      var hasVote = false;
      var infoTopic = {};
      var infoVote = {};
      ...
      interactionList.forEach(function(item, index) {
        if (item.type == 1) {
          hasTopic = true;
          infoTopic = item;
        }
        if (item.type == 2) {
          hasVote = true;
          infoVote = item;
          voteEndTime = utils.formatTime(item.endDate);
          endTime = item.endDate;
          voteImage = item.interactionPictureUrl;
        }
      });

      var reverlAllFlag = res.resultContent.contentUrl.indexOf('<img') != -1;
      ...
      that.setData({
        ...
        voteFlag: res.resultContent.voteFlag,
        voteMap: res.resultContent.voteMap,
        hasTopic: hasTopic,
        hasVote: hasVote,
        infoTopic: infoTopic,
        infoVote: infoVote,
        voteEndTime: voteEndTime,
        topicRecommentList:res.resultContent.rollComments,
        voteImage:voteImage,
        recommandArticleList: res.resultContent.recommandArticle ? res.resultContent.recommandArticle : [],
        reverlAllFlag: res.resultContent.reverlAllFlag || reverlAllFlag,
        author:author
      });
      ...
      console.log(that.data.articleInfoResult.contentUrl,'that.data.articleInfoResult.contentUrl')
      var temp = WxParse.wxParse('article', 'html', that.data.articleInfoResult.contentUrl, that, 0);
      that.setData({
        article: temp
      })

      if (res.resultContent.articleExt && res.resultContent.articleExt.videoUrl) {
        
        that.setData({
          video: res.resultContent.articleExt.videoUrl,
          videoHeight:'400rpx'
        })
      }
      var date = new Date();
     
      if (Date.parse(date) >= endTime && hasVote) {
        
        res.resultContent.voteFlag = true;
        that.setData({
          voteEnd: true
        })
      }
      if (res.resultContent.voteFlag) {


        let x = res.resultContent.voteMap[that.data.infoVote.interactionItems[0].id];
        let y = res.resultContent.voteMap[that.data.infoVote.interactionItems[1].id];
        that.setData({
          radioVote: (x + y != 0) ? [(x * 100 / (x + y)).toFixed(2), (y * 100 / (x + y)).toFixed(2)] : [0, 0]
  ...
  parseSuperComments(res, newComment) {
    ...
    this.setData({
      ...
      var that = this
      this.data.dataList.map(function(item,index){
      //canvas部分
      if(item.shareImage == '') {

        wx.downloadFile({
          url: item.fromUserLogoPath,
          success: (res) => {
            let userLogo = res.tempFilePath
              wx.downloadFile({
                url: item.pictureUrl ? item.pictureUrl :"https://dev-article.s3.cn-north-1.amazonaws.com.cn/imageUrl/20181012152825.png",
                success: (res) => {
                  let picture = res.tempFilePath
                  let canvas = "canvas" + index % 5
                  ...
                  context.clip();
                  context.drawImage(userLogo, 20, 20, d, d);
                  context.restore();
                  ...
                  context.fillText(item.fromUserName, 80, 38);
                  context.restore();
                  ...
                  context.setTextAlign('left');
                  context.fillText(item.createDate, 80, 63);
                  context.restore();
                  ...
                  context.setTextAlign('left');
                  context.fillText(item.content, 80, 93);
                  context.restore();

                  context.drawImage(picture, 80, 110, 90, 150);
                  ...
                  context.draw(false, (res) => {
                    ...
                    return wx.canvasToTempFilePath({
                      ...
                      width: 500,
                      height: 400,
                      canvasId: 'canvas' + index % 5,
                      success: function (res) {
                        
                        let index = (item.index)
                        
                        item.shareImage = res.tempFilePath
                        that.setData({ ['dataList[' + index + ']']: item })
